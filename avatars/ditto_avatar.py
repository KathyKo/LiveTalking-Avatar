...
def _closest_idle_sequence_index(history_small, idle_small):
    if not idle_small:
        return 0
    if not history_small:
        return 0
    history = list(history_small)
    return min(
        range(len(idle_small)),
        key=lambda next_index: sum(
            float(np.mean(np.abs(
                frame - idle_small[(next_index - len(history) + offset) % len(idle_small)]
            )))
            for offset, frame in enumerate(history)
        ),
    )
...
class DittoReal(BaseAvatar):
    ...
    def _run_chunk(self, audio, chunksize, keep_frames=None, vad_frames=None):
        ...
        if self._utt_active:
            ...
            for i in range(chunksize[1]):
                keep = i < keep_frames
                frame_seq = self._utt_frame_seq if keep else None
                if keep:
                    self._utt_frame_seq += 1
                self._frame_keep.put((keep, frame_seq))
    ...
    def _flush_tail(self):
        ...
        for keep_frames in _tail_frame_counts(
            ...
            vad_window = self._vad_buf[pos:pos + _SPLIT_LEN]
            if len(window) < _SPLIT_LEN:
                window = np.pad(window, (0, _SPLIT_LEN - len(window)))
                vad_window = np.pad(vad_window, (0, _SPLIT_LEN - len(vad_window)))
            self._run_chunk(
                window,
                _CHUNKSIZE,
    ...
    def _on_frame(self, frame_rgb):
        ...
        if self._drop_ditto_frames:
            ...
            return
        try:
            keep_frame, frame_seq = self._frame_keep.get_nowait()
        except queue.Empty:
            self._prof_frames_drop += 1
            self._prof_log()
            return
